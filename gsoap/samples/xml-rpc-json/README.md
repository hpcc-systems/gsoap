...
JSON RPC and JSON REST code.  For example, a JSON REST call in C++ to a JSON
...
~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~{.cpp}
    ...
    // bind to port 8080
    if (!soap_valid_socket(soap_bind(ctx, NULL, 8080, 1)))
      ... // error, stop
...
Note that we only have a queue of one with `soap_bind(ctx, NULL, 8080, 1)` which
rejects multiple requests.  Enlarge the queue as you wish, but beware that
larger queues can lead to server access problems.  Also set appropriate timeouts
and message length limits, see the [tutorials](https://www.genivia.com/tutorials.html#How_to_harden_your_application's_robustness_with_timeouts_and_error_handlers)
