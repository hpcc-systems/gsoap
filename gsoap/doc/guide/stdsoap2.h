...
/// When this macro is defined at compile time (undefined by default), the XSI-compliant POSIX version of `strerror_r` is expected instead of the GNU-specific `strerror_r`.
/**
This macro affects compilation of `stdsoap2.c` and `stdsoap2.cpp`.  On some systems such as Musl Linux the XSI-compliant POSIX version of `strerror_r` is not detectable at compile time and this may lead to compile-time errors for `strerror_r` mismatched return types.  In this case, compile with `-D WITH_POSIX_STRERROR_R`.  This feature requires version 2.8.139 or greater.

@par Example, when building the library and tools:

    ./configure CFLAGS=-DWITH_POSIX_STRERROR_R CXXFLAGS=-DWITH_POSIX_STRERROR_R

@par Example, when building from source:

    c++ -D WITH_POSIX_STRERROR_R -o client stdsoap2.cpp soapC.cpp soapClient.cpp client.cpp
*/
#define WITH_POSIX_STRERROR_R
...
~~~{.cpp}
soap_set_omode(soap, SOAP_ENC_MIME);
...
~~~{.cpp}
soap_clr_omode(soap, SOAP_ENC_MIME);
~~~
...
/// `::soap_mode` XML input/output flag value to serialize C/C++ data structures as XML trees without id-href or id-ref multi-references and ignores these attributes during deserialization.
/**
For data serialization to XML, the serializer duplicates co-referenced data in the XML output and automatically breaks data structure cycles to prevent infinite serialization loops.  For deserialization from XML, the deserializer ignores id and href/ref reference attributes in messages received.  This flag is the opposite of `#SOAP_XML_GRAPH`
...
@note This mode flag is always active when the engine source code is compiled with `#WITH_NOIDREF`.

@see `#SOAP_XML_GRAPH`, `#WITH_NOIDREF`.
*/
#define SOAP_XML_TREE 0x00020000

/// `::soap_mode` XML input/output flag value to serialize C/C++ (cyclic) data structures in XML with id-ref references for co-referenced data and to accurately serialize data structure cycles, accept id-ref references in XML received to (re)construct the C/C++ (co-referenced and cyclic) data structures,

/**
This flag is intended to accurately (de)serialize C/C++ data structures, including cyclic data references e.g. through the use of pointers in the data structures.  This (de)serialization mode is normally active with SOAP 1.1/1.2 serialization to preserve and process the XML data binding to C/C++, including multi-referenced objects in XML.  This flag is the opposite of `#SOAP_XML_TREE`
...
/// `::soap_ssl_flags` flag value to enable SSL v3 but disable TLS, should be used for legacy purposes only (not recommended)
/**
This flag can be used in combination with other SSL and TLS protocol flags, using `|` (bit-wise or).
...
/// `::soap_ssl_flags` flag value to enable both SSL v3 and TLS 1 (TLS 1.0 to 1.3) (not recommended)
/**
This flag can be used in combination with other SSL and TLS protocol flags, using `|` (bit-wise or).
...
/// `::soap_ssl_flags` flag value to enable TLS v1 (TLS 1.0 to 1.3) and to disable SSL v3
/**
This flag can be used in combination with other SSL and TLS protocol flags, using `|` (bit-wise or).
