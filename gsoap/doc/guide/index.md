...
<div align="right"><i>Copyright (c) 2000-2025, Genivia Inc.<br>All rights reserved.</i></div>
...
* **HTTP**: HTTP 1.0/1.1, IPv4 and IPv6, HTTPS (requires OpenSSL 3.0 or the
  latest or GNUTLS or WolfSSL), cookies, authentication, Zlib deflate and gzip
  compression, and connecting through HTTP proxies.
...
* OpenSSL 3.0 or the latest (OpenSSL 1.1 and earlier versions are also
  supported but not recommended) or GNUTLS or WolfSSL, and the Zlib libraries
  to enable SSL (HTTPS) and compression. These libraries are available for most
  platforms and are often already installed.
...
Or after running the configure script the <i>`config.h`</i> file can be used to compile from source:
...
Alternatively, compile <i>`gsoap/stdsoap2.c`</i> from source:
...
Alternatively, compile <i>`gsoap/stdsoap2.cpp`</i> from source:
...
* Endpoint URL: <i>`http://example.com/service.cgi`</i>
...
<i>`name`</i> with <b>`soapcpp2 -c -p name`</b> instead of <i>`soap`</i> as the
file name prefix.  This option is useful for C-based projects to avoid clashes.
This option is not useful for C++ projects in combination with options
<b>`-i`</b>, <b>`-j`</b> and <b>`-q`</b> that also affect file and class
naming.
...
For example:
    ...
    soapcpp2 -c -p foo file.h

This saves C source code files `fooStub.h`, `fooH.h`, `fooC.c`, and so on.
...
option to the C compiler to rename this file to include instead of
<i>`soapH.h`</i>.
...
After invoking the soapcpp2 tool on an interface header file description of a service to generate <i>`soapStub`</i>, <i>`soapH.h`</i>, and <i>`soapC.cpp`</i> for the XML serializers, and <i>`soapClient.cpp`</i> for the client stub functions, the client application is compiled in C++ as follows:
    ...
    c++ -o myclient myclient.cpp soapC.cpp soapClient.cpp -lgsoap++

Alternatively, compile <i>`gsoap/stdsoap2.cpp`</i> from source:

    c++ -o myclient myclient.cpp soapC.cpp soapClient.cpp stdsoap2.cpp

or after running the configure script the <i>`config.h`</i> file can be used to compile from source:

    c++ -DHAVE_CONFIG_H -I. -I.. -o myclient myclient.cpp stdsoap2.cpp soapC.cpp soapClient.cpp

For C we use <b>`soapcpp2 -c`</b> option <b>`-c`</b> to generate C source code that is compiled with:

    cc -o myclient myclient.c soapC.c soapClient.c -lgsoap

Alternatively, compile <i>`gsoap/stdsoap2.c`</i> from source:

    cc -o myclient myclient.c soapC.c soapClient.c stdsoap2.c 

Or after running the configure script the <i>`config.h`</i> file can be used to compile from source:

    cc -DHAVE_CONFIG_H -I. -I.. -o myclient myclient.c stdsoap2.c soapC.c soapClient.c
...
or after running the configure script the <i>`config.h`</i> file can be used to compile from source:
    ...
    c++ -DHAVE_CONFIG_H -I. -I.. -o myserver myserver.cpp stdsoap2.cpp soapC.cpp soapServer.cpp
...
or after running the configure script the <i>`config.h`</i> file can be used to compile from source:
    ...
    cc -DHAVE_CONFIG_H -I. -I.. -o myserver myserver.c stdsoap2.c soapC.c soapServer.c
...
access the container's elements, it must be comparable (equal/unequal), and be
pre-incrementable (`++it`).  The const iterator is used by its
soapcpp2-generated serializer to send a sequence of XML element values.  The
`insert` method is used to populate a container with `Container::iterator i =
...
not what we want.  If your current clients and services are based on
...
binary XML data elements, it is very easy to adopt MTOM by renaming the binary
types to `xop__Include` and using the `#SOAP_ENC_MTOM` flag with the SOAP 1.2
namespace.
...
~~~{.cpp}
    soap_clr_omode(soap, SOAP_ENC_MTOM);
~~~
...
`#SOAP_EOF`                 | Unexpected end of file, no input, transmission interrupted, timed out, or max input length exceeded (value of `SOAP_EOF` is `EOF`)
...
* `::soap::recv_maxlength` the maximum length of messages received, 2GB by default, a `#SOAP_EOF` error is produced when the input message exceeds the specified maximum length.  Set to zero to permit unlimited length (not recommended for server-side applications).
...
The `::soap::transfer_timeout` is new since 2.8.48 and limits the time a message send and a message receive operation can take.  This value should be used in combination with `::soap::send_timeout` and `::soap::recv_timeout` for accurate timeout control.

Related to timeouts is the `::soap::recv_maxlength` value, when non-zero limits the length of messages that can be received in bytes in total in decompressed form (message length includes HTTP headers and HTTP chunk size fields).  The value of `::soap::recv_maxlength` is 2GB by default.  Set to zero to permit unlimited length (not recommended for server-side applications).
...
error will be returned ("end of file or no input").  Negative timeout values
measure timeouts in microseconds, for example:
...
together with TLS 1.0 to 1.3, use `#SOAP_SSLv3_TLSv1` with
`::soap_ssl_server_context`.  To use TLS 1.0, 1.1, 1.2, 1.3 (by negotiation)
use `#SOAP_TLSv1`.  To use TLS 1.2, 1.3 use `#SOAP_TLSv1_2 |
#SOAP_TLSv1_3`. To use SSL v3 only use `#SOAP_SSLv3`.
...
To utilize HTTPS at the client side, you need the OpenSSL library.
...
Since release version 2.8.20 SSL v3 is disabled. To enable SSL v3 together with TLS 1.0 and higher, use `#SOAP_SSLv3_TLSv1` with `::soap_ssl_server_context`. To use TLS 1.0, 1.1, 1.2, 1.3 (by negotiation) use `#SOAP_TLSv1`. To use TLS 1.2, 1.3 only use `#SOAP_TLSv1_2 | SOAP_TLSv1_3`. To use SSL v3 only use `#SOAP_SSLv3`.
...
Or after running the configure script the <i>`config.h`</i> file can be used to compile from source:
     ...
     cc -DHAVE_CONFIG_H -I. -I.. -c -DWITH_NONAMESPACES stdsoap2.c
...
<i>Copyright (c) 2000-2025, Robert A. van Engelen, Genivia Inc.<br>All rights reserved.</i>
